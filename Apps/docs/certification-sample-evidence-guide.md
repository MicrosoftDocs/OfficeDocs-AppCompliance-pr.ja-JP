...
1. **証拠提出: 認定** 評価のスコープ内の各コントロールに必要な証拠の完全なセット。 各コントロールの証拠の例を以下に示します。
...
- 証拠ガイドラインの例: 証拠は、メーリングリストへのサブスクリプションを表示し、新しくリリースされた脆弱性のセキュリティ ソースを手動で確認する方法 (JIRA または Azure DevOps など)、古いソフトウェアを検索するツール (たとえば、古いソフトウェア ライブラリを探している場合は Snyk である可能性があります)、アクティビティのタイムスタンプを使用して適切に追跡する必要があります。 または、古いソフトウェアを識別する認証済みスキャンを使用して Nessus である可能性があります。
