...
>Azure Cache Redis を使用します。キャッシュ内のデータの最大寿命は 300 分です。 キャッシュ データアイテムは AES256 で暗号化されます。 アプリ サービスだけが SSL 経由で Azure Cache Redis に接続できます。接続文字列は Azure 環境変数に格納されます。
