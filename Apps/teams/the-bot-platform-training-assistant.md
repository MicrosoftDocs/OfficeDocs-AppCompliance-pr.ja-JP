...
description: トレーニング アシスタント、そのデータ処理ポリシー、Microsoft Cloud App Security アプリ カタログ情報、CSA STAR レジストリ内のセキュリティ/コンプライアンス情報に関して利用可能なすべてのセキュリティおよびコンプライアンス情報。
...
| アプリのコア機能 | このデジタル アシスタントは、ユーザーの設定と作業を行う上で知る必要があるすべてをカバーする一連のトレーニング モジュールをTeams。 各モジュールには、特定のチュートリアル ビデオとトレーニング コンテンツが含まれているので、スタッフは Microsoft Teams で利用できる機能の広さ、およびそれらを使用する方法を十分に認識しています。 |
