...
description: Proconnect for Sitecore で使用可能なすべてのセキュリティおよびコンプライアンス情報、そのデータ処理ポリシー、Microsoft Cloud App Security アプリ カタログ情報、CSA STAR レジストリのセキュリティ/コンプライアンス情報。
...
| アプリのコア機能 | ワークフロー発行の更新に関するインスタント アラートを使用して、コンテンツ作成エクスペリエンス &amp; を強化します。 このコネクタには、アラートに対する適切なアクションも含まれるので、コンテンツ作成者は作業を迅速に実行できます。 このコネクタは、コミュニケーションの透明性を確立し、サイト/ページのパフォーマンスに関する分析情報も含まれています。 |
| 会社の本社所在地 | カナダ |
| アプリ情報ページ | |
