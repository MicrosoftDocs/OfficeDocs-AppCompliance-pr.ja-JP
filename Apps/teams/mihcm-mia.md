...
| アプリには、バックアップと復元の戦略を含む、文書化された障害復旧計画がありますか? | 必要 |
| 環境で従来のマルウェア対策保護またはアプリケーションコントロールを使用していますか? | ApplicationControls |
| セキュリティの脆弱性をインデントおよびリスクランク付けするプロセスが確立されていますか? | 必要 |
| パッチの適用に関するサービス レベル契約 (SLA) を管理するポリシーはありますか? | 必要 |
...
| アプリには、要求に応じて個人の個人データを削除する機能がありますか? | 必要 |
...
>|:------------------------|:--------------------|:------------------------------------|
>| User.Read | 委任 | ユーザーを一意に識別するには |
>| Group.Read.All | 委任 | ユーザーの Microsoft Planner タスクを読み取る |
>| MailboxSettings.Read | アプリケーション | ユーザーのタイム ゾーンを読み取る |
>| Calendars.Read | アプリケーション | ユーザーのスケジュールされた予定表イベントを読み取る |
