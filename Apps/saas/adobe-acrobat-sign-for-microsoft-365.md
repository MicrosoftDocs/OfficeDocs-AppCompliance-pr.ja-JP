---
title: Adobe Acrobat Sign for Microsoft 365 by Adobe のアプリケーション情報
ms.author: elmalova
author: elenamalova
manager: tonybal
ms.date: 04/20/2022
...
description: Adobe Acrobat Sign for Microsoft 365、データ処理ポリシー、Microsoft Cloud App Security アプリ カタログ情報、CSA STAR レジストリのセキュリティ/コンプライアンス情報に関するすべての利用可能なセキュリティ情報とコンプライアンス情報。
...
* <a href="https://appsource.microsoft.com/product/web-apps/adobe.adobe_sign_msft_saas_offer" target="_blank">AppSource で表示する</a>
...
| **Information** | **応答** |
|:----------------|:-------------|
| アプリ名 | Adobe Acrobat Sign for Microsoft 365 |
...
| アプリのコア機能 | Adobe Sign を Office-Outlook-Team に追加すると、ドキュメント署名ワークフローを高速に移動できます。 これらの製品から、Microsoft の優先電子署名ソリューションを使用して、法的に拘束力のある電子署名と承認&#8212;ドキュメントを送信し、署名することができます。 |
| 会社の本社の場所 | 米国 オブ アメリカ |
| アプリ情報ページ | [https://helpx.adobe.com/sign/using/microsoft-teams-integrat...](https://helpx.adobe.com/sign/using/microsoft-teams-integration-user-guide.html) |
...
| アプリによって処理されるデータは何ですか? | アクセス トークンを更新するには、現在のトークンの有効期限が切れている場合。 たとえば、ユーザーが署名&quot;の送信ウィンドウに&quot;いて、非アクティブな状態を長すぎる場合、ユーザーがアクティブになっているときに新しいトークンを更新する必要があります。 |
| アプリは TLS 1.1 以降をサポートしていますか? | はい |
| アプリまたは基になるインフラストラクチャには、Microsoft の顧客データは格納されますか? | はい |
| データベースに格納されているデータは何ですか? | アクセス トークンを更新するには、現在のトークンの有効期限が切れている場合。 たとえば、ユーザーが署名&quot;の送信ウィンドウに&quot;いて、非アクティブな状態を長すぎる場合、ユーザーがアクティブになっているときに新しいトークンを更新する必要があります。 |
...
| プライバシー ポリシーの URL | https://helpx.adobe.com/sign/help/adobesign_gdpr_compliance.html |
...
| アプリで上記のライブラリのいずれかを使用していない場合、どの認証ライブラリまたはライブラリが使用されますか? | adal (MSAL に移行する予定) |
...
>|:------------------------|:--------------------|:------------------------------------|:--------------------|
>| Mail.ReadWrite | 委任 | 添付ドキュメント、送信者と受信者の電子メール、および電子メールから Adobe へのメッセージ コンテンツを設定して署名用に送信します。 これは、ユーザーが Adobe Sign でこれらのフィールドを再入力する時間を節約するためです。 契約が署名されると、ユーザーが電子メールを送信するための新しい電子メールが自動的に作成され、トランザクションが完了したことを受信者に通知します。 Adobe Sign は添付ファイルを一時ファイルとして保存します。有効期限は 24 時間です。 | [ea36b867-ca67-45fd-a61b-d2be86273167](../azure/ea36b867-ca67-45fd-a61b-d2be86273167.md) |
>| People.Read | 委任 | 署名&quot;の送信エクスペリエンスでメール アドレスを&quot;自動入力するには、最初の文字を入力して、ユーザーがメール全体を入力する必要はありません。 Adobe Sign は、受信者のメールと displayName のみを契約に保存します。 | [ea36b867-ca67-45fd-a61b-d2be86273167](../azure/ea36b867-ca67-45fd-a61b-d2be86273167.md) |
>| User.Read | 委任 | ユーザーのプロファイルを読み取り、Adobe Sign を使用できるように、ユーザーのプロファイル (基本的には、その電子メールと userId) をデータベースに一致させます。 ユーザーのプロファイルを読み取り、Adobe Sign を使用できるように、ユーザーのプロファイル (基本的には、その電子メールと userId) をデータベースに一致させます。 | [ea36b867-ca67-45fd-a61b-d2be86273167](../azure/ea36b867-ca67-45fd-a61b-d2be86273167.md) |
>| offline_access | 委任 | アクセス トークンを更新するには、現在のトークンの有効期限が切れている場合。 たとえば、ユーザーが署名&quot;の送信ウィンドウに&quot;いて、非アクティブな状態を長すぎる場合、ユーザーがアクティブになっているときに新しいトークンを更新する必要があります。 アクセス トークンを更新するには、現在のトークンの有効期限が切れている場合。 たとえば、ユーザーが署名&quot;の送信ウィンドウに&quot;いて、非アクティブな状態を長すぎる場合、ユーザーがアクティブになっているときに新しいトークンを更新する必要があります。 | [ea36b867-ca67-45fd-a61b-d2be86273167](../azure/ea36b867-ca67-45fd-a61b-d2be86273167.md) |
>| openid | 委任 | 電子メールと UserId。 ユーザーが Adobe Sign アプリを使用するアクセス許可に同意していることを確認するためにサインインする。 電子メールは、Adobe Sign のユーザーの一意の識別子です。 そのユーザーのすべてのアクティビティを Adobe Sign レコードにマップできるように、電子メール ID が保存されます。 | [ea36b867-ca67-45fd-a61b-d2be86273167](../azure/ea36b867-ca67-45fd-a61b-d2be86273167.md) |
